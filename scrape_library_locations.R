...
extract_info <- function(x){
  ...
  lib_loc <- str_sub(x, ind2[1] + 2, ind2[1] + 23)
  ind4 <- stringr::str_locate(lib_loc,",")
  lib_lat <- as.numeric(str_sub(lib_loc, 1, ind4[1] - 1))
  lib_lng <- as.numeric(str_sub(lib_loc, ind4[1] + 1))
...
df <- purrr::map(gmap_links, extract_info) |> purrr::list_rbind()
